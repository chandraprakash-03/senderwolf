...
const smtpSchema = z
    .object({
        ...
        dev: z.boolean().optional().default(false), // Dev Preview Server Mode
        proxy: z.object({
            host: z.string(),
            port: z.number(),
            type: z.enum(['socks5', 'socks4', 'http']).optional().default('socks5'),
            auth: z.object({
                user: z.string(),
                pass: z.string()
            }).optional()
        }).optional(),
        transport: z.string().optional(), // 'stream', 'json', 'ses', etc.
    })
    .optional();
...
const mailSchema = z
    .object({
        ...
        replyTo: z.union([email, z.array(email), z.string()]).optional(),
        subject: z.string().optional(), // Optional if subjects array is provided
        subjects: z.array(z.string()).optional(), // For A/B testing
        ...
        attachments: z
            .array(
                z.union([
                    ...
                    }).refine(data => data.url || data.href, { message: "Either url or href must be provided" }),
        ...
        /** BIMI configuration */
        bimi: z.object({
            /** SVG URL for the BIMI logo */
            location: z.string().url(),
            /** DNS selector for BIMI (optional) */
        ...
        /** List headers (List-Unsubscribe, etc.) */
        list: z.record(z.union([z.string(), z.object({ url: z.string(), comment: z.string().optional() })])).optional(),
        /** S/MIME encryption and signing config */
        smime: z.any().optional(),
        /** PGP encryption and signing config */
        pgp: z.any().optional(),
    })
    .refine((data) => data.html || data.text || data.amp || data.calendar !== undefined || data.react !== undefined || data.vue !== undefined || data.pug !== undefined || data.ejs !== undefined || data.handlebars !== undefined, {
        message: "Either 'html', 'text', 'amp', 'calendar', 'react', 'vue', 'pug', 'ejs', or 'handlebars' must be provided in mail body.",
        path: ["html"],
    });
...
const queueSchema = z
    .object({
        store: z.any(), // The QueueStore instance
    })
    .optional();
...
export function validateInput(input) {
    const mainSchema = z.object({
        ...
        failover: z.array(smtpSchema).optional(),
    });

    return mainSchema.parse(input);
}
