...
export class SMTPConnectionPool extends EventEmitter {
    ...
    async getConnection(config) {
        ...
        if (existingConnection && existingConnection.isIdle() && !existingConnection.isExpired() && existingConnection.isHealthy()) {
        ...
        // If existing connection is unhealthy, clean it up
    ...
    async createConnection(config, key) {
        ...
        // Pre-increment before connect — decremented by 'close' event or on failure
        ...
        try {
            ...
            // Undo the pre-increment since connection never established (BUG-1 fix)
            this.activeConnections = Math.max(0, this.activeConnections - 1);
            throw error;
        }
    }
