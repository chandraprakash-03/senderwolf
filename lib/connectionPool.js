...
export class SMTPConnectionPool extends EventEmitter {
    ...
    async getConnection(config) {
        ...
        if (connectionSet) {
            for (const conn of connectionSet) {
                if (conn.isIdle() && !conn.isExpired() && conn.isHealthy()) {
        ...
        if (existingConnection) {
            ...
            // Clear any pending idle timeout when connection is reused (PERF-2)
            if (existingConnection._idleTimer) {
                clearTimeout(existingConnection._idleTimer);
                existingConnection._idleTimer = null;
            }
    ...
    async createConnection(config, key) {
        ...
        try {
            ...
            let connectionSet = this.connections.get(key);
            if (!connectionSet) {
                connectionSet = new Set();
                this.connections.set(key, connectionSet);
            }
    ...
    processQueue() {
        ...
        for (let i = 0; i < this.queue.length; i++) {
            ...
            const connectionSet = this.connections.get(item.key);
            let hasIdle = false;
            ...
            if (hasIdle || this.activeConnections < this.options.maxConnections) {
                indexToProcess = i;
                break;
            }
        }
        ...
        if (indexToProcess !== -1) {
            const item = this.queue.splice(indexToProcess, 1)[0];
            this.getConnection(item.config)
                .then(item.resolve)
                .catch(item.reject);
        }
    }
    ...
    releaseConnection(connection) {
        ...
        // Clear any previous idle timeout (PERF-2: prevent timer accumulation)
        if (connection._idleTimer) {
            clearTimeout(connection._idleTimer);
        }

        // Set idle timeout
        connection._idleTimer = setTimeout(() => {
    ...
    async close() {
        ...
        const allConnections = Array.from(this.connections.values()).flatMap(set => Array.from(set));
        const closePromises = allConnections.map(conn => conn.close());
    ...
    getStats() {
        ...
        return {
            ...
            idleConnections: allConnections.filter(c => c.isIdle()).length,
...
class PooledSMTPConnection extends EventEmitter {
    ...
    /**
     * Check if the connection is still healthy (socket not destroyed/errored)
     */
    isHealthy() {
        return !this.closed && this.client && this.client.socket && !this.client.socket.destroyed;
    }
