...
import { StreamTransport, JSONTransport } from "./transports.js";
...
export async function coreSend(smtp, mailOptions) {
    ...
    try {
        ...
        if (smtp.transport === 'stream') {
            const transport = new StreamTransport(smtp);
            const client = new SMTPClient(smtp); // for MIME building
            const result = await transport.send(mailOptions, async (opts) => client.buildTransportParts(opts));
            cb.recordSuccess();
            return result;
        } else if (smtp.transport === 'json') {
            const transport = new JSONTransport(smtp);
            const result = await transport.send(mailOptions);
            cb.recordSuccess();
            return result;
        }
    ...
    try {
        ...
        const merged = {
            smtp: {
                ...
                auth: (input.smtp?.auth?.user || config.user || input.smtp?.auth?.type === 'oauth2' || input.smtp?.auth?.type === 'xoauth2') ? {
                ...
                transport: input.smtp?.transport || config.transport,
            },
            mail: {
                ...
                compile: input.mail?.compile ?? config.compile ?? false,
                react: input.mail?.react,
                vue: input.mail?.vue,
                pug: input.mail?.pug,
                ejs: input.mail?.ejs,
                handlebars: input.mail?.handlebars,
                list: input.mail?.list,
                smime: input.mail?.smime,
                pgp: input.mail?.pgp,
            },
            retry: input.retry || {},
            queue: input.queue,
        ...
        // Handle React Email Support
        if (mail.react) {
            try {
                const { render } = await import('@react-email/render');
                mail.html = await render(mail.react);
                if (!mail.text) {
                    mail.text = await render(mail.react, { plainText: true });
                }
            } catch (err) {
                throw new Error("To use React Email, you must install '@react-email/render'. Run: npm install @react-email/render");
            }
        }

        // Handle Vue Email Support
        if (mail.vue) {
            try {
                const { render } = await import('@vue-email/render');
                mail.html = await render(mail.vue);
            } catch (err) {
                throw new Error("To use Vue Email, you must install '@vue-email/render'. Run: npm install @vue-email/render");
            }
        }

        // Handle Pug, EJS, and Handlebars
        if (mail.pug) {
            try {
                const pugModule = await import('pug');
                const pugLib = pugModule.default || pugModule;
                const compiledFunction = pugLib.compileFile(mail.pug.template, mail.pug.options || {});
                mail.html = compiledFunction(mail.pug.locals || {});
            } catch (err) {
                throw new Error("To use Pug, you must install 'pug'. Run: npm install pug\\nDetails: " + err.message);
            }
        }
        ...
        if (mail.ejs) {
            try {
                ...
                const ejsLib = ejsModule.default || ejsModule;
                mail.html = await ejsLib.renderFile(mail.ejs.template, mail.ejs.locals || {}, mail.ejs.options || {});
            } catch (err) {
                throw new Error("To use EJS, you must install 'ejs'. Run: npm install ejs\\nDetails: " + err.message);
            }
        }
        ...
        if (mail.handlebars) {
            try {
                ...
                const hbsLib = hbsModule.default || hbsModule;
                const source = await fs.readFile(mail.handlebars.template, 'utf8');
                const template = hbsLib.compile(source, mail.handlebars.options || {});
                mail.html = template(mail.handlebars.locals || {});
            } catch (err) {
                throw new Error("To use Handlebars, you must install 'handlebars'. Run: npm install handlebars\\nDetails: " + err.message);
            }
        }
        ...
        // Pre-process attachments: sanitize filenames, infer names, and fetch URLs
        if (mail.attachments && mail.attachments.length > 0) {
            mail.attachments = await Promise.all(mail.attachments.map(async att => {
                ...
                if (url && !att.content) {
                    try {
                        ...
                        att.content = Buffer.from(arrayBuffer);
                        if (!filename) {
                            filename = url.split('/').pop().split('?')[0] || 'downloaded_attachment';
                        }
                    } catch (fetchErr) {
                        throw new Error(`Error fetching attachment from ${url}: ${fetchErr.message}`);
                    }
                }
        ...
        // Handle Compile mode
        if (mail.compile) {
            logger.info(`Compile mode enabled for ${mail.to}. Returning raw MIME string.`);
            const client = new SMTPClient(smtp);
            const message = await client.buildMimeMessage(mailOptions);
            return {
                success: true,
                messageId: mailOptions.messageId || 'compiled',
                compile: true,
                message,
                attempts: 0
            };
        ...
        for (let i = 0; i < configsToTry.length; i++) {
            ...
            try {
                ...
                return {
                    ...
                    ...extraResult,
                    ...(typeof messageId === 'object' ? messageId : {}),
                    attempts: totalAttempts + attempts,
                    scheduled: waitTime > 0,
                    transport: (typeof messageId === 'object' && messageId.transport) ? messageId.transport : (i === 0 ? 'primary' : `failover-${i}`)
                };
            } catch (error) {
                lastError = error;
