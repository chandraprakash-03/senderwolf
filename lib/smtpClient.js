...
import { applySMIME, applyPGP } from "./crypto.js";
...
export class SMTPClient {
    ...
    async sendCommand(command, expectCode = "250") {
        if (this.debug) this.logger.info(`C: ${command}`);
        this.socket.write(command + "\r\n");
        const response = await this.readResponse();
        if (this.debug) this.logger.info(`S: ${response}`);
    ...
    async authenticate() {
        ...
        // Handle STARTTLS opportunistically or if required
        if (!this.secure) {
            if (this.capabilities.has('STARTTLS')) {
                ...
                throw new Error('STARTTLS is required but not supported by the server');
            }
        }
    ...
    _buildMimeParts(mailOptions) {
        const messageId = mailOptions.messageId || `<${Date.now()}.${Math.random().toString(36).substring(2, 11)}@${os.hostname()}>`;
        ...
        // SEC-6: Sanitize header values to prevent CRLF injection
        const sanitizeHeaderValue = (val) => val === undefined || val === null ? '' : String(val).replace(/\r\n|\r|\n/g, ' ').trim();

        const toRecipients = Array.isArray(mailOptions.to) ? mailOptions.to : [mailOptions.to];

        parts.push(`From: ${sanitizeHeaderValue(mailOptions.fromHeader)}\r\n`);
        parts.push(`To: ${sanitizeHeaderValue(toRecipients.join(", "))}\r\n`);

        // Add CC header (but not BCC for privacy)
        if (mailOptions.cc) {
            const ccRecipients = Array.isArray(mailOptions.cc) ? mailOptions.cc : [mailOptions.cc];
            parts.push(`Cc: ${sanitizeHeaderValue(ccRecipients.join(", "))}\r\n`);
        }

        // Add Reply-To if specified (supports array or string)
        if (mailOptions.replyTo) {
            const replyToVal = Array.isArray(mailOptions.replyTo) ? mailOptions.replyTo.join(", ") : mailOptions.replyTo;
            parts.push(`Reply-To: ${sanitizeHeaderValue(replyToVal)}\r\n`);
        }

        parts.push(`Subject: ${sanitizeHeaderValue(mailOptions.subject)}\r\n`);
        parts.push(`Message-ID: ${sanitizeHeaderValue(messageId)}\r\n`);
        ...
        // Add List-* headers
        if (mailOptions.list) {
            for (const [key, value] of Object.entries(mailOptions.list)) {
                let headerValue = '';
                if (typeof value === 'string') {
                    headerValue = `<${value}>`;
                } else if (typeof value === 'object' && value.url) {
                    headerValue = `<${value.url}>`;
                    if (value.comment) {
        ...
        if (hasRegularAttachments) {
            for (const attachment of regularAttachments) {
                ...
                let useStream = !this.dkim && (attachment.path || (attachment.content && typeof attachment.content.read === "function"));
                ...
                if (attachment.path) {
                    ...
                    if (!useStream || typeof attachment.content.read !== "function") {
                ...
                // Sanitize filename to prevent SMTP relay issues or injection (W5 fix)
                if (filename) {
                    filename = filename.replace(/[\r\n"<>|]/g, '').trim();
                } else if (attachment.cid) {
                    // Fallback filename if only CID is provided
                    filename = `inline-${attachment.cid.replace(/[^a-zA-Z0-9]/g, '')}`;
                } else {
                ...
                if (attachment.cid) {
                    parts.push(`Content-ID: <${attachment.cid.replace(/[<>]/g, '')}>\r\n`);
                    // If CID is present, default to inline unless explicitly set to attachment (W4 fix)
                ...
                if (useStream) {
                    parts.push({
                        type: 'stream',
                        stream: attachment.path ? fs.createReadStream(attachment.path) : attachment.content
                    });
                    continue;
                }
                ...
                if (Buffer.isBuffer(fileContent)) {
                    ...
                    throw new AttachmentError("Stream attachments must be read into a Buffer before sending when DKIM is enabled.");
                } else {
                    base64Content = Buffer.from(String(fileContent)).toString("base64");
                }
    ...
    async buildTransportParts(mailOptions) {
        const { parts, messageId } = this._buildMimeParts(mailOptions);
        const hasStreams = parts.some(p => typeof p !== 'string');
        
        if (mailOptions.smime || mailOptions.pgp) {
             if (hasStreams) {
                 throw new Error("S/MIME and PGP are not supported with stream attachments.");
             }
             let message = parts.join('');
             if (mailOptions.smime) message = applySMIME(message, mailOptions.smime);
             if (mailOptions.pgp) message = applyPGP(message, mailOptions.pgp);
             return { parts: [message], messageId };
        }
        return { parts, messageId };
    }

    async buildMimeMessage(mailOptions) {
        const { parts } = this._buildMimeParts(mailOptions);
        const hasStreams = parts.some(p => typeof p !== 'string');

        if (hasStreams) {
            throw new Error("Stream attachments are not supported in compile mode.");
        }

        let message = parts.join('');

        if (mailOptions.smime) {
             message = applySMIME(message, mailOptions.smime);
        } else if (mailOptions.pgp) {
             message = applyPGP(message, mailOptions.pgp);
        }
        ...
        if (hasStreams) {
            for (const part of parts) {
                if (typeof part === 'string') {
                    const stuffed = part.replace(/\r\n\./g, '\r\n..');
                    if (!this.socket.write(stuffed)) {
                        await new Promise(resolve => this.socket.once('drain', resolve));
                    }
                } else if (part.type === 'stream') {
                    await this._writeBase64Stream(part.stream);
                }
            }
            this.socket.write(terminator);
        } else {
    ...
    /**
     * Helper to safely stream and base64-encode large payloads
     */
    async _writeBase64Stream(readableStream) {
        return new Promise((resolve, reject) => {
            ...
            const handleData = (chunk) => {
                ...
                const completeLen = buffer.length - (buffer.length % 3);
                const toEncode = buffer.subarray(0, completeLen);
                remainder = buffer.subarray(completeLen);
                ...
                if (toEncode.length > 0) {
                    ...
                    if (!this.socket.write(output)) {
                        readableStream.pause();
                        this.socket.once('drain', () => readableStream.resume());
                    }
                }
            };
            ...
            readableStream.on('end', () => {
                ...
                if (remainder.length > 0) {
                    const b64 = remainder.toString('base64');
                    for (let i = 0; i < b64.length; i++) {
                        output += b64[i];
                        lineLength++;
                        if (lineLength === 76) {
    ...
    extractEmail(emailString) {
        ...
        let email = match ? match[1] : emailString;
        
        // Apply punycode to domain part for SMTPUTF8
        const parts = email.split('@');
        if (parts.length === 2) {
            email = `${parts[0]}@${domainToASCII(parts[1])}`;
        }
