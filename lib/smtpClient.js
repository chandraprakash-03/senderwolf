...
export class SMTPClient {
    ...
    async authLogin() {
        if (!this.capabilities.has('AUTH') && !this.capabilities.has('LOGIN')) {
            throw new AuthenticationError('Server does not support LOGIN authentication');
        }
    ...
    async authPlain() {
        if (!this.capabilities.has('AUTH') && !this.capabilities.has('PLAIN')) {
            throw new AuthenticationError('Server does not support PLAIN authentication');
        }
    ...
    async sendMail(mailOptions) {
        ...
        if (mailOptions.html || mailOptions.text || calendarAttachments.length > 0) {
            const altBoundary = `----=_Part_Alt_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`;
            ...
            // 3. Calendar third (most preferred in clients like Outlook/Gmail)
            for (const cal of calendarAttachments) {
                message += `--${altBoundary}\r\n`;
                message += `Content-Type: ${cal.contentType}\r\n`;
                message += `Content-Transfer-Encoding: base64\r\n\r\n`;

                let fileContent = cal.content;
                if (cal.path) fileContent = fs.readFileSync(cal.path);
