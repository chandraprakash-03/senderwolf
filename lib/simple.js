...
export function createMailer(config = {}) {
    ...
    return {
        /**
         * Register a middleware function for a specific step
         * @param {string} step - The step to hook into (e.g., 'compile')
         * @param {Function} handler - The middleware function
         */
        use(step, handler) {
            middlewares.push({ step, handler });
            return this;
        },
        ...
        async send(options) {
            ...
            // Execute middlewares for the 'compile' step
            for (const mw of middlewares) {
                if (mw.step === 'compile') {
                    await mw.handler(merged.mail);
                }
            }
