...
function startServer(port = 3000, attempt = 0, sslOptions = null) {
    if (server) return;
    ...
    if (sslOptions && sslOptions.key && sslOptions.cert) {
        try {
            ...
            logger.error(`Dev Server failed to load SSL certificates: ${err.message}. Falling back to HTTP.`);
            server = http.createServer(requestHandler);
        }
    } else {
        // Fallback to HTTP for local development when no certificates are provided
        server = http.createServer(requestHandler);
    }
    ...
    server.on('error', (e) => {
        if (e.code === 'EADDRINUSE') {
            ...
            if (attempt < MAX_PORT_RETRIES) {
                startServer(port + 1, attempt + 1, sslOptions);
            } else {
    ...
    server.listen(port, '127.0.0.1', () => {
        ...
        console.log(`\x1b[32m📧 Preview intercepted emails at \x1b[4m${protocol}://127.0.0.1:${port}\x1b[0m\n`);
    });
}
...
export async function captureEmail(mailOptions) {
    if (!server) {
        try {
            ...
            startServer(3000, 0, config.devServer?.ssl);
        } catch (err) {
            // If config fails to load, start with defaults
            startServer();
        }
    }
    ...
    const isHttps = server instanceof https.Server;
    const protocol = isHttps ? 'https' : 'http';
    logger.info(`Dev Server intercepted email to ${mailOptions.to}. View at ${protocol}://localhost:${displayPort}`);
